...
-(sqlite3 *)database
{
    return [self databaseForTableName:nil];
...
//            !sqlite3_open([[self staticDatabaseFilePath] UTF8String], &staticDataBase) == SQLITE_OK)
//		{
//			// Even though the open failed, call close to properly clean up resources.
//			NSAssert1(0, @"Failed to open database with message '%s'.", sqlite3_errmsg(database));
//			sqlite3_close(database);
//            sqlite3_close(staticDataBase);
//		}
//		else
//		{
...
- (sqlite3 *)databaseForTableName:(NSString *)tableName
{
	static BOOL first = YES;
...
- (void)deleteDatabase
{
    ...
    NSString *staticPath = [self staticDatabaseFilePath];
    [fm removeItemAtPath:staticPath error:NULL];
...
- (void)dealloc
{
    ...
    [staticDatabaseFilePath release];
	[super dealloc];
}
#pragma mark -
...
- (NSString *)staticDatabaseFilePath
{
