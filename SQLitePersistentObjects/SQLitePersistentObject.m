...
+ (double)performSQLAggregation: (NSString *)query, ...
{
	...
	^{
		sqlite3 *database = [[SQLiteInstanceManager sharedManager] databaseForTableName:[self tableName]];
...
+(NSArray *)findByCriteriaFull:(NSString *)criteriaString
{
	...
	[SQLitePersistentObject performUsingDBOperationQueue:
     ^{
         result = [self _findByCriteria:criteriaString];
     }];
...
//批量存储时，将其放在一个事务中进行，加快插入操作的速度
+ (void)saveBatchItems:(NSArray *)aryItems
{
     [SQLitePersistentObject performUsingDBOperationQueue:
      ^{
          [[self class] tableCheck];
          ...
          @try {
              ...
              for(id item in aryItems)
              {
                  [item _saveInTransaction];
              }
...
- (void)_saveInTransaction
{
	...
	if (dirty)
	{
		...
		sqlite3 *database = [[SQLiteInstanceManager sharedManager] databaseForTableName:[[self class] tableName]];
...
+ (void)deleteBatchItems:(NSArray *)aryItems
{
    [[self class] tableCheck];
    sqlite3 *database = [[SQLiteInstanceManager sharedManager] databaseForTableName:[self tableName]];
    @try {
...
+ (void)createIndices:(NSArray *)aryIndices
{
    [SQLitePersistentObject performUsingDBOperationQueue:
     ^{
         sqlite3 *database = [[SQLiteInstanceManager sharedManager] databaseForTableName:[self tableName]];
         NSArray *theIndices = [NSArray arrayWithObject:aryIndices];
         if (theIndices != nil)
         {
             if ([theIndices count] > 0)
             {
